...
### 规划
- 下一阶段确定为 **P5E.6 知识地图产品重置**：主入口从“Wiki”收束为“知识地图”，Chat 继续负责提问和搜索，资料卡负责原文目录与定位，概念页与个人笔记负责可编辑的知识层。导入仅建立本地基础索引；AI 概念提取改为按资料或按 Chat 主题触发、候选审查后写入。全库批量整理、深度解读、修复、预算和 Provider 路由降为高级维护。此项仅记录已确认计划，尚未实现。
