    ...
    <section className={`wiki-plan-card ${plan.status}`} aria-label="Wiki 整理计划">
          ...
          return (
            <details className={`wiki-plan-change ${change.kind}`} key={change.change_id}>
              <summary>
                ...
                <span><strong>{change.title}</strong><small>{change.page_type === "wiki_concept" ? "概念页" : "实体页"} · {change.source_count} 个原文位置</small></span>
      ...
      <footer className="wiki-plan-actions">
        ...
        <div>
          ...
          {plan.status === "planned" && onApply && <button className="primary-button" disabled={busy || applicable === 0} onClick={onApply}><Check size={16} />{busy ? "正在写入" : "确认并生成"}</button>}
        </div>
      </footer>
    </section>
  );
