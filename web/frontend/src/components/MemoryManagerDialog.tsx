...
export function MemoryManagerDialog({ onClose, memoryEnabled }: { onClose: () => void; memoryEnabled: boolean }) {
  ...
  return <section className="memory-manager" role="dialog" aria-modal="true" aria-label="管理个人知识">
    ...
    {loading ? <LoadingState label="正在读取个人知识…" /> : <main className="memory-manager-main">
      {tab === "knowledge" && <>
        <div className="memory-toolbar"><label><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索已确认知识" /></label><DropdownSelect ariaLabel="作用域筛选" value={scope} onChange={(value) => setScope(value as typeof scope)} options={[{ value: "all", label: "全部作用域" }, { value: "global", label: "全局" }, { value: "library", label: "当前资料库" }]} /><button className="quiet-button" disabled={!memoryEnabled} onClick={() => setDraft(emptyDraft)}><Plus size={15} />新增</button><button className="quiet-button" onClick={() => void exportKnowledge()}><Download size={15} />导出</button></div>
        <section className="memory-editor"><label><span>作用域</span><DropdownSelect ariaLabel="作用域" value={draft.scope} disabled={Boolean(draft.id) || !memoryEnabled} onChange={(value) => setDraft({ ...draft, scope: value as "global" | "library" })} options={[{ value: "library", label: "当前资料库" }, { value: "global", label: "所有资料库" }]} /></label><label><span>类型</span><DropdownSelect ariaLabel="类型" value={draft.kind} disabled={!memoryEnabled} onChange={(value) => setDraft({ ...draft, kind: value as KnowledgeKind })} options={Object.entries(kindLabels).map(([value, label]) => ({ value, label }))} /></label><label className="wide"><span>标题</span><input value={draft.title} disabled={!memoryEnabled} maxLength={120} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></label><label className="wide"><span>内容</span><textarea rows={4} value={draft.content} disabled={!memoryEnabled} maxLength={5000} onChange={(event) => setDraft({ ...draft, content: event.target.value })} /></label><footer><label><input type="checkbox" checked={draft.pinned} disabled={!memoryEnabled} onChange={(event) => setDraft({ ...draft, pinned: event.target.checked })} />置顶</label>{draft.id && <button className="quiet-button" onClick={() => setDraft(emptyDraft)}>取消编辑</button>}<button className="primary-button" disabled={!memoryEnabled || working || !draft.title.trim() || !draft.content.trim()} onClick={() => void saveItem()}><Check size={15} />保存</button></footer></section>
      ...
      {tab === "candidates" && <div className="candidate-list">{candidates.map((candidate) => {
        const edited = candidateDraft?.id === candidate.id ? candidateDraft : candidate;
        return <article key={candidate.id}><header><span>{candidate.operation === "update" ? "更新建议" : "新知识候选"} · {kindLabels[candidate.kind]}</span><strong>{candidate.title}</strong><small>{candidate.reason}</small></header><details><summary>查看证据与编辑</summary><div className="candidate-editor"><label><span>作用域</span><DropdownSelect ariaLabel="作用域" value={edited.scope} disabled={!memoryEnabled} onChange={(value) => setCandidateDraft({ ...edited, scope: value as "global" | "library" })} options={[{ value: "library", label: "当前资料库" }, { value: "global", label: "所有资料库" }]} /></label><label><span>类型</span><DropdownSelect ariaLabel="类型" value={edited.kind} disabled={!memoryEnabled} onChange={(value) => setCandidateDraft({ ...edited, kind: value as KnowledgeKind })} options={Object.entries(kindLabels).map(([value, label]) => ({ value, label }))} /></label><label className="wide"><span>标题</span><input value={edited.title} disabled={!memoryEnabled} onChange={(event) => setCandidateDraft({ ...edited, title: event.target.value })} /></label><label className="wide"><span>内容</span><textarea rows={4} value={edited.content} disabled={!memoryEnabled} onChange={(event) => setCandidateDraft({ ...edited, content: event.target.value })} /></label><div className="candidate-evidence">{candidate.evidence.length ? candidate.evidence.map((item, index) => <p key={index}>{String(item.excerpt || item.source_id || "学习记录")}</p>) : <p>没有额外证据片段。</p>}</div></div></details><footer><button className="quiet-button" disabled={working} onClick={() => void resolveCandidate("reject", candidate)}>拒绝</button><button className="primary-button" disabled={working || !memoryEnabled} onClick={() => void resolveCandidate("confirm", candidate)}><Check size={15} />确认并保存</button></footer></article>;
