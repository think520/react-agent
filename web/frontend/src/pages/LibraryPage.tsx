...
import { BookmarkCheck, CheckCircle2, FilePlus2, FileText, FolderOpen, Plus, Quote, RefreshCw, Search, ShieldCheck, Trash2, Upload, Wrench, X } from "lucide-react";
...
import type { DocumentSection, DocumentSummary, WikiHealth } from "../types";
...
export function LibraryPage() {
  ...
  useEffect(() => { void loadDocuments(); }, [collection]);
  ...
  async function organizeWiki() {
    ...
    try {
      ...
      setNotice(result.archived_count
        ? `已归档 ${result.archived_count} 个重复页面，并重建规范索引。`
        : "Wiki 已检查并重建规范索引，没有发现重复页面。");
      await loadDocuments();
    } catch (reason) {
      ...
      <div className="page-container library-container">
        <header className="page-heading">
          ...
          <div className="heading-actions">
            ...
            {collection === "wiki" && <button className="primary-button" onClick={() => void openWikiMaintenance()}><Wrench size={16} />维护 Wiki</button>}
