 ...
 *
 * ④ 两套阅读实现合一的第 2 步：这里只剩薄壳——解析路由参数与 search params、
 * 加载资料列表（顶栏标题、上一份/下一份、标签条要用）、渲染顶栏与标签条；
 * 文章主体整体交给 components/DocumentReader。
 */

import { useCallback, useEffect, useRef, useState } from "react";
import { ArrowLeft, ArrowRight, CheckCircle2, Pencil, RefreshCw, Sparkles } from "lucide-react";
...
import type { DocumentExtractionStatus, DocumentSection, DocumentSummary } from "../types";
...
export function ReaderPage() {
  ...
  const { activeLibrary } = useOutletContext<AppOutletContext>();

  const [documents, setDocuments] = useState<DocumentSummary[]>([]);
  // 阅读区（DocumentReader）加载完小节后回传的副本：顶栏的「提取概念」按它判断
  // 按钮可用与正文内容，读取语义与搬之前一致。
  ...
  return (
    <section className="page-scroll reader-page" ref={pageRef}>
      {confirmElement}
      ...
      {editingDocumentId && selected && (
        <DocumentEditor
          ...
          onSaved={() => {
            ...
            void loadDocuments();
            // 阅读区自己按这个信号重取小节（原来是在这里直接 setSections）。
