...
| 条目 | 审计编号 | 动作 | 状态 |
|---|---|---|---|
| 静态托管路径穿越 | P0-4 | `spa_fallback` 加 resolve 包含性判断，越界 404 | 待做 |
| 工具沙箱边界与写保护 | P0-5 + P0-6 | 派发前剥离模型保留键、`workspace`/`cwd` 无条件覆盖；写路径白名单；trace 凭据形态脱敏 | 待做 |
| 中文 token 估算 | P0-7 | 自研分档保守估算（CJK≈1.2/字、ASCII≈1/5，系数来自本项目语料实测） | 待做 |
| 删概念 500 | P0-11 | 从属行 `ON DELETE CASCADE` + 服务层兜底 | 待做 |
...
| chunk_id 稳定性 | P1-21 | Python `hash()` → `_stable_hash` | 待做 |
| reset 一致性 | P1-28 | `/kb reset` 清理 `manifest.json` | 待做 |

#### 批次二 · 接线（已定方案，批次一完成后开始）

hooks 最小接线（结果上限与消毒落 `after_tool`、权限门落 `before_tool`，记忆注入不动，加守护测试）→ 上下文压缩两级（L1 工具结果转可重取 marker、L2 确定性 checkpoint + turn 边界切点 + 序列化后配对兜底，无 checkpoint 不压缩）→ append-only 事件表 + `Last-Event-ID` 续传（含 P1-11 悬空 tool_call 与断线宽限取消）→ specialist 上 Web → 向量补建驱动 → grep 非文本可见降级。
